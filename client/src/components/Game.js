...
const Game = () => {
    ...
    const handleShare = () => {
        ...
        let verb = "played";
        if (gameState === "won") {
            if (score === 1) verb = getRandom(["was in perfect harmony with", "hit a high note on", "composed a masterpiece on", "was instrumental in beating", "conducted a clinic on", "hit a grand slam (not the baseball kind) on", "played first chair on", "was pitch-perfect on", "cranked the volume to 11 on", "unlocked the secret track on"]);
            else if (score <= 3) verb = getRandom(["found the rhythm on", "stayed on beat for", "soundchecked", "tuned up on", "headlined", "remixed", "amplified", "orchestrated a win on", "jammed out to", "trebled my way through"]);
            else if (score === 4) verb = getRandom(["stayed in the groove on", "didn't miss a beat on", "slow-danced through", "found the right key on", "harmonized with", "improvised a win on", "kept the tempo on", "scratched the surface of"]);
            else if (score === 5) verb = getRandom(["barely made the encore on", "survived the mosh pit on", "strummed past", "limped to the finish line on", "beat the drum for", "was a bit sharp/flat on", "almost hit a sour note on", "fumbled the sheet music for"]);
            else if (score === 6) verb = getRandom(["narrowly escaped the gong on", "pulled a miracle out of the hat on", "clutched the final chord on", "barely heard the music on", "just about stayed in tune for", "sweated through the solo on", "found the hidden track on", "survived the feedback on"]);
        } else {
            verb = getRandom(["got gonged by", "was totally tone-deaf on", "broke a string on", "forgot the lyrics to", "hit a sour note on", "got stage fright on", "was out of sync with", "lost the beat on", "tanked the audition for", "got booed off the stage on", "experienced technical difficulties with", "got stuck in a loop on", "lost the sheet music for", "fumbled the mic on", "got upstaged by", "was a one-hit wonder on", "got drowned out by", "faced the music on", "missed the cue for", "was off-key for", "got skipped on", "blew the speakers on"]);
        }
